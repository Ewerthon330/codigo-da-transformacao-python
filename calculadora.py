...
from collections import (
    deque,
)  # Importa deque para gerenciar o histórico de forma eficiente
...
def limpar_ultima_nota():
    ...
    if campos_entrada:
        ...
        messagebox.showinfo(
            "Limpar", "A última nota foi limpa. Por favor, digite o novo valor."
        )
    else:
...
def cadastrar_e_calcular():
        ...
        for campo in campos_entrada:
            ...
            if not valor_str:
                messagebox.showerror(
                    "Erro de Entrada", "Por favor, preencha todas as 4 notas."
                )
                return
            ...
            if 0 <= valor_nota <= 10:
                ...
                messagebox.showerror(
                    "Erro de Validação", "As notas devem estar entre 0 e 10 (0 a 10)."
                )
                return

    except ValueError:
        messagebox.showerror(
            "Erro de Entrada", "Valores inválidos. Digite apenas números."
        )
        return
    ...
    if media_final >= nota_minima_aprovacao:
        resultado_texto += "STATUS: 🎉 ALUNO PASSOU DE ANO!"
...
if __name__ == "__main__":
    ...
    menu_arquivo.add_command(
        label="Mostrar Histórico (20)", command=mostrar_historico
    )  # Opção para Histórico
    menu_arquivo.add_separator()
    menu_arquivo.add_command(label="Sair", command=janela.quit)

    # Widgets da Tela Principal
    tk.Label(janela, text="Insira as Notas (0 a 10):", font=("Arial", 10, "bold")).grid(
    ...
    # Nota 4
    tk.Label(janela, text="Nota 4:").grid(row=4, column=0, padx=5, pady=5, sticky="e")
    entry_nota4 = tk.Entry(janela, width=15)
    ...
    campos_entrada = [entry_nota1, entry_nota2, entry_nota3, entry_nota4]

    # Botão para Acionar o Cálculo
    tk.Button(
        janela,
        text="Calcular Média",
        command=cadastrar_e_calcular,
        bg="#4C93AF",
        fg="white",
        font=("Arial", 10, "bold"),
    ...
    tk.Button(
        janela,
        text="Limpar Última Nota",
        command=limpar_ultima_nota,
        bg="#FF0062",
        fg="white",
